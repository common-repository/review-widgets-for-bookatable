...
{
case 'open':
update_option($trustindex_pm_bookatable->get_option_name('rate-us') , 'hide', false);
$url = 'https://wordpress.org/support/plugin/'. $trustindex_pm_bookatable->get_plugin_slug() . '/reviews/?rate=5#new-post';
header('Location: '. $url);
die;
case 'later':
$time = time() + (30 * 86400);
update_option($trustindex_pm_bookatable->get_option_name('rate-us') , $time, false);
break;
case 'hide':
update_option($trustindex_pm_bookatable->get_option_name('rate-us') , 'hide', false);
break;
...
{
delete_option($trustindex_pm_bookatable->get_option_name('proxy-check'));
header('Location: admin.php?page=' . sanitize_text_field($_GET['page']) .'&tab=' . sanitize_text_field($_GET['tab']));
exit;
}
$tabs = [];
if($trustindex_pm_bookatable->is_trustindex_connected())
{
...
if($proxy->is_enabled())
{
$opt_name = $trustindex_pm_bookatable->get_option_name('proxy-check');
...
<?php echo TrustindexPlugin::___("Widgets for Bookatable Reviews"); ?>
<a href="https://www.trustindex.io/ti-redirect.php?a=sys&c=wp-bookatable-l" target="_blank" title="Trustindex" class="ti-pull-right">
<img src="<?php echo $trustindex_pm_bookatable->get_plugin_file_url('static/img/trustindex.svg'); ?>" />
</a>
</h1>
...
href="<?php echo admin_url('admin.php?page='.$trustindex_pm_bookatable->get_plugin_slug().'/settings.php&tab='. esc_attr($action)); ?>"
...
href="<?php echo admin_url('admin.php?page='.$trustindex_pm_bookatable->get_plugin_slug().'/settings.php&tab='. esc_attr($tab)); ?>"
><?php echo esc_html($tab_name); ?></a>
<?php endforeach; ?>
</div>
<?php endif; ?>
