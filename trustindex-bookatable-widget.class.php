...
class TrustindexWidget_bookatable extends WP_Widget {
...
{
parent::__construct(
'trustindex_bookatable_widget',
'Trustindex - Bookatable reviews widget',
array(
'classname' => 'trustindex-widget',
'description' => TrustindexPlugin::___('Embed Bookatable reviews fast and easily into your WordPress site. Increase SEO, trust and sales using Bookatable reviews.')
)
);
}
...
if ($trustindex_pm_bookatable->is_enabled())
{
extract($args);
echo $before_widget;
$was_error = false;
...
{
echo $trustindex_pm_bookatable->get_trustindex_widget($instance['ti-widget-ID']);
}
elseif($trustindex_pm_bookatable->is_noreg_linked() && $trustindex_pm_bookatable->is_noreg_table_exists())
{
echo $trustindex_pm_bookatable->get_noreg_list_reviews();
}
else
{
echo TrustindexPlugin::get_alertbox(
"error",
" in <strong>".TrustindexPlugin::___('Widgets for Bookatable Reviews')."</strong> plugin<br /><br />"
.TrustindexPlugin::___('Please fill out <strong>all the required fields</strong> in the <a href="%s">widget settings</a> page', [admin_url('admin.php?page='.$trustindex_pm_bookatable->get_plugin_slug().'/settings.php')]),
false
);
}
echo $after_widget;
...
global $wp_version;
global $trustindex_pm_bookatable;
$ti_widgets = $trustindex_pm_bookatable->get_trustindex_widgets();
...
<?php if ($trustindex_pm_bookatable->is_trustindex_connected()): ?>
<?php if ($ti_widgets): ?>
...
<img src="<?php echo $trustindex_pm_bookatable->get_plugin_file_url('static/img/help-where-is-id.jpg'); ?>" alt="ID column here: <?php echo 'https://admin.trustindex.io/'; ?>widget" />
</div>
<?php else: ?>
<?php echo TrustindexPlugin::get_alertbox("warning",
...
. TrustindexPlugin::___("Go to <a href='%s'>plugin setup page</a> to complete the one-step setup guide and enjoy the full functionalization!", array(admin_url('admin.php?page='.$trustindex_pm_bookatable->get_plugin_slug().'/settings.php&tab=setup_trustindex')))
); ?>
<?php endif; ?>
</div>
<?php
