...
{
check_admin_referer( 'connect-reg_'.$trustindex_pm_bookatable->get_plugin_slug());
...
{
$server_output = $trustindex_pm_bookatable->connect_trustindex_api(
array(
"signin" => array(
...
{
check_admin_referer( 'disconnect-reg_'.$trustindex_pm_bookatable->get_plugin_slug());
delete_option($trustindex_pm_bookatable->get_option_name("subscription-id"));
setcookie('ti-success', 'disconnected', time() + 60, "/");
header('Location: admin.php?page=' . sanitize_text_field($_GET['page']) . '&tab=' . sanitize_text_field($_GET['tab']));
exit;
}
$trustindex_subscription_id = $trustindex_pm_bookatable->is_trustindex_connected();
$widget_number = $trustindex_pm_bookatable->get_trustindex_widget_number();
?>
...
<p><strong><?php echo TrustindexPlugin::___('You can connect your %s with your Trustindex account, and can display your widgets easier.', [ 'Widgets for Bookatable Reviews' ]); ?></strong></p>
...
<?php wp_nonce_field( 'disconnect-reg_'.$trustindex_pm_bookatable->get_plugin_slug() ); ?>
...
<?php wp_nonce_field( 'connect-reg_'.$trustindex_pm_bookatable->get_plugin_slug() ); ?>
...
<a class="btn-text" href="https://www.trustindex.io/ti-redirect.php?a=sys&c=wp-bookatable-4" target="_blank"><?php echo TrustindexPlugin::___('Create a new Trustindex account');?></a>
</p>
</form>
...
<code class="code-ti-w-<?php echo esc_attr($wc_i .'-'. $wi_num); ?>">[<?php echo $trustindex_pm_bookatable->get_shortcode_name(); ?> data-widget-id="<?php echo esc_attr($w['id']); ?>"]</code>
