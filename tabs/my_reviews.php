...
{
update_option( $trustindex_pm_bookatable->get_option_name('download-timestamp') , time() + (86400 * 2), false);
}
}
if(isset($_POST['save-highlight']))
{
check_admin_referer( 'save-noreg_'.$trustindex_pm_bookatable->get_plugin_slug(), '_wpnonce_highlight_save' );
$id = null;
$start = null;
$length = null;
if(isset($_POST['id']))
...
$wpdb->query("UPDATE `". $trustindex_pm_bookatable->get_noreg_tablename() ."` SET highlight = '$highlight' WHERE id = '$id'");
}
exit;
}
$reviews = [];
if($trustindex_pm_bookatable->is_noreg_linked() && $trustindex_pm_bookatable->is_noreg_table_exists())
{
...
if($trustindex_pm_bookatable->is_ten_scale_rating_platform())
{
return '<div class="ti-rating-box">'. $trustindex_pm_bookatable->formatTenRating($score) .'</div>';
}
$text = "";
$link = "https://cdn.trustindex.io/assets/platform/".ucfirst("bookatable")."/star/";
if(!is_numeric($score))
{
return $text;
}
...
');
$download_timestamp = get_option($trustindex_pm_bookatable->get_option_name('download-timestamp'), time() - 1);
?>
<?php if(!$trustindex_pm_bookatable->is_noreg_linked()): ?>
<div class="notice notice-warning" style="margin-left: 0">
<p><?php echo TrustindexPlugin::___("Connect your %s platform to download reviews.", ["Bookatable"]); ?></p>
</div>
<?php else: ?>
...
<?php endif; ?>
<?php if(!$trustindex_pm_bookatable->is_trustindex_connected() && $download_timestamp < time()): ?>
<div class="notice notice-error" style="margin: 0 0 15px 0">
<p>
<?php echo TrustindexPlugin::___("Don't want to waste your time by updating your reviews every week? <a href='%s' target='_blank'>Create a free Trustindex account! »</a>", [ 'https://www.trustindex.io/ti-redirect.php?a=sys&c=wp-bookatable-l' ]); ?>
</p>
</div>
<?php endif; ?>
...
<td class="text-center source-<?php echo ucfirst("bookatable") ?>"><?php echo trustindex_plugin_write_rating_stars($review->rating); ?></td>
<td class="text-center"><?php echo esc_html($review->date); ?></td>
<td><div class="ti-review-content"><?php echo $trustindex_pm_bookatable->getReviewHtml($review); ?></div></td>
<td>
...
<?php wp_nonce_field( 'save-noreg_'.$trustindex_pm_bookatable->get_plugin_slug(), '_wpnonce_highlight_save' ); ?>
